...
occurrences <- occurrences[complete.cases(occ),]
```
...
```{r}
occ <- cbind(occurrences$decimalLongitude, occurrences$decimalLatitude)

eps <- 5
minpts <- 20
dbscan.res <- dbscan(occ, eps=eps, MinPts=minpts)
```
...
```{r}
plot(occ, col=dbscan.res$cluster)
```
