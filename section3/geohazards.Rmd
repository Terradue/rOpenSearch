...
1. Load the required packages with:
...
2. Define the OpenSearch description URL

```{r}
osd.url <- "http://eo-virtual-archive4.esa.int/search/ASA_IM__0P/description"
```
3. Get the list of OpenSearch response types:

```{r}
GetOSResponseFormats()
```

4. Set the OpenSearch response type to use:

```{r}
response.type <- "application/rdf+xml"
```

5. get the queryables dataframe from the OpenSearch description URL
